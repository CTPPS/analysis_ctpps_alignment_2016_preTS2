...
int main()
{
	...
	auto input_alignments = alignmentCollection.find(fillInfo.alignmentTag)->second;
	...
	for (const auto &ad : armData)
	{
		...
		for (const string rp : { "N", "F"} )
		{
			...
			double p1_init = 0.10;
			double p2_init = 0.07;
			double p3_init = (rp == "N") ? input_alignments[ad.rp_id_N].sh_y : input_alignments[ad.rp_id_F].sh_y;
